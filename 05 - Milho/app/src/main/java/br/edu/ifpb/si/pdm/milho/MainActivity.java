...
//        });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_orcamento) {
            ...
            Toast.makeText(this, "Jaênia Sousa", Toast.LENGTH_SHORT).show();
        }else if (id == R.id.action_zerar){
            this.cadastro.clear();
            this.atualizaLista();
        }else if(id == R.id.action_ord_descricao){
            this.cadastro.ordenarDescricao();
            this.atualizaLista();
        }else if(id == R.id.action_ord_quantidade){
            this.cadastro.ordenarQuantidade();
            this.atualizaLista();
        }else if(id == R.id.action_ord_valor_unitario){
            this.cadastro.ordenarValorUnitario();
            this.atualizaLista();
        }else if(id == R.id.action_ord_valor_total){
            this.cadastro.ordenarValorTotal();
            this.atualizaLista();
        }else if(id == R.id.action_ord_data){
            this.cadastro.ordenarData();
            this.atualizaLista();
        }
    ...
    private class ListaLongClickListener implements AdapterView.OnItemLongClickListener {
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            dialogo.setTitle("Tem certeza que deseja excluir o item?");
            dialogo.setPositiveButton(R.string.action_ok, new BotaoExcluirListener(position));
            dialogo.setNeutralButton(R.string.action_cancel, null);
            dialogo.show();

            return true;

            //Mostrar total do item
            /*Gasto g = MainActivity.this.cadastro.get(position);
    ...
    private class BotaoExcluirListener implements DialogInterface.OnClickListener{
        private int posGasto;

        public BotaoExcluirListener(int posGasto){
            this.posGasto = posGasto;
        }
        @Override
        public void onClick(DialogInterface dialog, int which) {
            MainActivity.this.cadastro.excluir(posGasto);
            MainActivity.this.atualizaLista();
            Log.i("GASTO", Integer.toString(this.posGasto));
        }
    }
}
